...
def parse_xlsx(content_base64):
    try:
        ...
        return pd.read_excel(io.BytesIO(decoded), names=COLUMN_NAMES, header=None)
    except Exception as e:
        raise ValueError(f"XLSX parsing error: {str(e)}")
...
def parse_log(content):
    try:
        ...
        raise ValueError(f"LOG parsing error: {str(e)}")
...
def merge_files(files):
    ...
    merged_df = merged_df.sort_values(
        by="timestamp", ascending=True, na_position="last"
    )

    output_csv = merged_df.to_csv(index=False, sep=";")

    return {"name": "merged_data.csv", "content": output_csv}
